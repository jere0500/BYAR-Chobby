...
local function SetupInfoButtonsPanel(leftInfo, rightInfo, battle, battleID, myUserName)
		...
		OnClick = {
			function()
				if currentMapName and config.gameConfig.link_particularMapPage ~= nil then
					WG.BrowserHandler.OpenUrl(config.gameConfig.link_particularMapPage(
					currentMapName))
				end
			end
		}
		...
		OnClick = {
			function()
				...
				WG.IntegerSelectorWindow.CreateIntegerSelectorWindow({
					...
					labelCaption =
					"Split the map start boxes vertically, with X percent of the map going to left and right start boxes.",
					...
					OnAccepted = function(integervalue)
						if battleLobby.name == "singleplayer" then
							...
							externalFunctions.AddStartRect(1, 200 - integervalue * 2, 0, 200,
								200)
						else
		...
		OnClick = {
			function()
				...
				WG.IntegerSelectorWindow.CreateIntegerSelectorWindow({
					...
					caption = "Change start boxes",
					labelCaption =
					"Split the map start boxes horizontally, with X percent of the map going to top and bottom start boxes.",
					imageFile = LUA_DIRNAME .. "images/startboxsplit_h.png",
					OnAccepted = function(integervalue)
						if battleLobby.name == "singleplayer" then
							externalFunctions.RemoveStartRect()
							externalFunctions.AddStartRect(0, 0, 0, 200, integervalue * 2)
							externalFunctions.AddStartRect(1, 0, 200 - integervalue * 2, 200,
								200)
						else
		...
		OnClick = {
			function()
				...
				WG.IntegerSelectorWindow.CreateIntegerSelectorWindow({
					...
					caption = "Change start boxes",
					labelCaption =
					"Split the map start boxes along the corners, with X percent of the map going to top left and bottom right start boxes.",
					imageFile = LUA_DIRNAME .. "images/startboxsplit_c1.png",
					OnAccepted = function(integervalue)
						if battleLobby.name == "singleplayer" then
							externalFunctions.RemoveStartRect()
							externalFunctions.AddStartRect(0, 0, 0, integervalue * 2,
								integervalue * 2)
							externalFunctions.AddStartRect(1, 200 - integervalue * 2,
								200 - integervalue * 2, 200, 200)
						else
		...
		OnClick = {
			function()
				...
				WG.IntegerSelectorWindow.CreateIntegerSelectorWindow({
					...
					caption = "Change start boxes",
					labelCaption =
					"Split the map start boxes along the corners, with X percent of the map going to bottom left and top right start boxes.",
					imageFile = LUA_DIRNAME .. "images/startboxsplit_c2.png",
					OnAccepted = function(integervalue)
						if battleLobby.name == "singleplayer" then
							externalFunctions.RemoveStartRect()
							externalFunctions.AddStartRect(0, 0, 200 - integervalue * 2,
								integervalue * 2, 200)
							externalFunctions.AddStartRect(1, 200 - integervalue * 2, 0, 200,
								integervalue * 2)
						else
		...
		OnClick = {
			function()
				...
				WG.IntegerSelectorWindow.CreateIntegerSelectorWindow({
					...
					caption = "Change start boxes",
					labelCaption =
					"Split the map start boxes along the corners, with X percent of the map going to all 4 corners.",
					...
					OnAccepted = function(integervalue)
						if battleLobby.name == "singleplayer" then
							...
							externalFunctions.AddStartRect(1, 200 - integervalue * 2, 0, 200,
								integervalue * 2)
							externalFunctions.AddStartRect(2, 0, 0, integervalue * 2,
								integervalue * 2)
							externalFunctions.AddStartRect(3, 200 - integervalue * 2,
								200 - integervalue * 2, 200, 200)
						else
		...
		OnClick = {
			function()
				...
				WG.IntegerSelectorWindow.CreateIntegerSelectorWindow({
					...
					caption = "Change start boxes",
					labelCaption =
					"Split the map start boxes along the sides, with X percent of the map going to all 4 sides.",
					imageFile = LUA_DIRNAME .. "images/startboxsplit_s.png",
					OnAccepted = function(integervalue)
						if battleLobby.name == "singleplayer" then
							externalFunctions.RemoveStartRect()
							externalFunctions.AddStartRect(0, 0, 100 - integervalue,
								integervalue * 2, 100 + integervalue)
							externalFunctions.AddStartRect(1, 200 - integervalue * 2,
								100 - integervalue, 200, 100 + integervalue)
							externalFunctions.AddStartRect(2, 100 - integervalue, 0,
								100 + integervalue, integervalue * 2)
							externalFunctions.AddStartRect(3, 100 - integervalue,
								200 - integervalue * 2, 100 + integervalue, 200)
						else
		...
		for i, rect in pairs(currentStartRects) do
			...
			if false then --could check width?
				...
				rect:SetPos(
					...
					math.floor(rect.parent.width * (rect.spadsSizes.right - rect.spadsSizes.left) /
					200),
					math.floor(rect.parent.height * (rect.spadsSizes.bottom - rect.spadsSizes.top) /
					200)
				) -- x,y,w,h
			end
			rect:Invalidate()
		...
		readyButton = Button:New {
			...
			OnClick = {
				function(readyButton)
					...
					local newReady = not battleLobby.userBattleStatus[battleLobby.myUserName]
					.isReady
			...
			function()
				...
				if battle.isRunning then
					if Spring.GetGameName() == "" then
						...
						WG.Chobby.ConfirmationPopup(RejoinBattleFunc,
							"Are you sure you want to leave your current game to rejoin this one?",
							nil, 315, 200)
					end
				else
					if battleLobby.name == "singleplayer" then
						...
						if Configuration.gameConfig.mapStartBoxes.singleplayerboxes then
							if currentStartRects ~= {} then
								Configuration.gameConfig.mapStartBoxes.singleplayerboxes =
								currentStartRects
							end
						end
						WG.Analytics.SendOnetimeEvent("lobby:singleplayer:skirmish:start")
						WG.SteamCoopHandler.AttemptGameStart("skirmish", battle.gameName,
							battle.mapName)
					else
			...
			function(obj)
				...
				if battleLobby.name == "singleplayer" then
					...
					local function becomeSpec()
						...
						battleLobby:_OnUpdateUserBattleStatus(battleLobby:GetMyUserName(),
							{ queuePos = 0 })                -- we proactive change our queuePos; because we don't reliable receicve s.battle.queue_status on fast clicking play/spectate
						battleLobby:SetBattleStatus({
							isSpectator = true,
							isReady = false
						})
					...
					if (battleStatus.isSpectator and not iAmQueued) or WG.Chobby.Configuration.queueExitConfirmPromptDoNotAskAgain then
						...
						WG.Chobby.ConfirmationPopup(becomeSpec, i18n("queue_exit_confirm"),
							"queueExitConfirmPromptDoNotAskAgain", 450, 225, i18n("yes"),
							i18n("cancel"))
					end
				end
	...
	local lblGame = Label:New {
		...
		OnResize = {
			function(obj, xSize, ySize)
				obj:SetCaption(StringUtilities.GetTruncatedStringWithDotDot(
				WG.Chobby.Configuration.gameConfig.ShortenNameString(battle.gameName), obj.font,
					xSize or obj.width))
			end
		}
	}
		...
		if battleInfo.mapName then
			...
			if isSingleplayer then
				...
				if Configuration.gameConfig and
					...
					startBoxes = Configuration.gameConfig.mapStartBoxes
					.selectStartBoxesForAllyTeamCount(startBoxes, allyTeamCount)
					if startBoxes then
						--externalFunctions.RemoveStartRect()
						for i = 1, allyTeamCount do
							if startBoxes[i] then
								externalFunctions.AddStartRect(i - 1,
									200 * startBoxes[i][1], 200 * startBoxes[i][2],
									200 * startBoxes[i][3], 200 * startBoxes[i][4])
							end
						end
					else
						-- !split v 20
					...
					Spring.Echo("No map startBoxes found or disabled for map", mapName, "teamcount:",
						allyTeamCount)
				end
			end
	...
	function externalFunctions.JoinedBattle(joinedBattleId, userName)
		if battleID ~= joinedBattleId or userName == battleLobby:GetMyUserName() then
			...
			if playersthatarentme == 1 and mynewbestfriend ~= "" then
				...
				if userInfo then
					Spring.PlaySoundFile("sounds/ring.wav",
						WG.Chobby.Configuration.menuNotificationVolume or 1) -- RING SOUND
		...
		local newStartRect = Window:New {
			...
			OnClick = {
				function(obj)
					...
					if math.abs(obj.oldSizes[1] - obj.x) < 1.0 and
						...
						startRectValues[tonumber(obj.caption)] = { ["left"] = l, ["top"] = t,
							["right"] = r, ["bottom"] = b }

						obj:Invalidate() --doesnt do much
						if battleLobby.name == "singleplayer" then
							WG.Chobby.Configuration.gameConfig.mapStartBoxes.addBox(l, t, r,
								b, obj.caption)
							obj.spadsSizes = { left = l, top = t, right = r, bottom = b, caption =
							obj.caption }
						else
							battleLobby:SayBattle(string.format("!addbox %d %d %d %d %s", l,
								t, r, b, obj.caption))
						end
					end
				end
			...
			OnResize = {
				...
				function(obj, width, height)
					...
					obj.width = math.max(obj.minWidth,
						math.min(obj.parent.width - obj.x - 1, obj.width))
					obj.height = math.max(obj.minHeight,
						math.min(obj.parent.height - obj.y - 1, obj.height))
				end
			},
		}
			...
			if teamIndex ~= -1 and teamIndex ~= -2 then
				...
				AddTeamButtons(
					...
					function(obj, x, y, button)
						...
						WG.PopupPreloader.ShowAiListWindow(battleLobby, battle.gameName,
							teamIndex, quickAddAi)
					end,
...
local function SetupVotePanel(votePanel, battle, battleID)
	...
	function externalFunctions.VoteEnd(message, success)
		...
		local text = ((success and WG.Chobby.Configuration:GetSuccessColor()) or WG.Chobby.Configuration:GetErrorColor()) ..
		oldTitle .. ((success and " Passed.") or " Failed.")
	...
	local nextButton = Button:New {
		...
		OnClick = {
			function(obj)
				...
				if nextPage then
					WG.Analytics.SendOnetimeEvent("lobby:singleplayer:skirmish:" .. pageConfig.name,
						selectedOptions[pageConfig.name])
...
local function SetupEasySetupPanel(mainWindow, standardSubPanel, setupData)
	...
	local function ApplyFunction(startGame)
		...
		if startGame then
			if haveMapAndGame then
				WG.SteamCoopHandler.AttemptGameStart("skirmish", battle.gameName, battle.mapName, nil,
					true)
			else
				Spring.Echo("Do something if map or game is missing")
			end
	...
	for i = 1, #pages do
		InitializeSetupPage(pages[i], screenHeight, pageConfigs[i], pages[i + 1], pages[i - 1], selectedOptions,
			ApplyFunction)
	end
end
...
local function InitializeControls(battleID, oldLobby, topPoportion, setupData)
	...
	local lblBattleTitle = Label:New {
		...
		OnResize = {
			function(obj, xSize, ySize)
				obj:SetCaption(StringUtilities.GetTruncatedStringWithDotDot(battleTitle, obj.font,
					obj.width))
			end
		}
	}
	...
	local function UpdateBattleTitle()
		if isSingleplayer then
			...
			local truncatedTitle = StringUtilities.GetTruncatedStringWithDotDot(battleTitle,
				lblBattleTitle.font, lblBattleTitle.width)
		...
		if Configuration.allEnginesRunnable or Configuration:IsValidEngineVersion(battle.engineVersion) then
			...
			local truncatedTitle = StringUtilities.GetTruncatedStringWithDotDot(battleTitle,
				lblBattleTitle.font, math.max(lblBattleTitle.width, 250))
			lblBattleTitle:SetCaption(truncatedTitle)
		else
		...
		if status.isSpectator ~= nil then
			...
			if status.isSpectator == false then
				...
				if status.queuePos then
					local Configuration = WG.Chobby.Configuration
					Spring.PlaySoundFile("sounds/BAR_Joined_queue6D_mixdown.wav",
						Configuration.menuNotificationVolume or 1, "ui")
				end
	...
	local function ParseForVotingSaidBattle(userName, message)
		...
		if newlycalledvote or string.match(message, "Vote in progress") then
			...
			local title = string.sub(message, string.find(message, ' "', nil, true) + 2,
				string.find(message, '" ', nil, true) - 1)
	...
	local function OnSaidBattle(listener, userName, message)
		...
		local myUserName = battleLobby:GetMyUserName()
		local iAmMentioned = myUserName and userName ~= myUserName and
		string.find(message, myUserName, nil, true)
	...
	singleplayerWrapper = Control:New {
		...
		OnParent = {
			function(obj)
				...
				battleLobby:SetBattleState(WG.Chobby.Configuration:GetPlayerName(), singleplayerGame,
					defaultMap, "Skirmish Battle")
				...
				if not (setupData and WG.Chobby.Configuration.simplifiedSkirmishSetup) and singleplayerDefault then
					...
					local function AddAI(counter, shortName, version, allyTeam, side, color)
						...
						if WG.Chobby.Configuration.simpleAiList and WG.Chobby.Configuration.gameConfig.GetAiSimpleName then
							fullName = WG.Chobby.Configuration.gameConfig.GetAiSimpleName(
							fullName)
					...
					if singleplayerDefault.startboxes then
						for startboxindex, startboxcoords in pairs(singleplayerDefault.startboxes) do
							...
							local infoHandler = mainWindowFunctions.GetInfoHandler()
							--Spring.Utilities.TableEcho(infoHandler)
							infoHandler.AddStartRect(startboxindex, startboxcoords[1],
								startboxcoords[2], startboxcoords[3], startboxcoords[4])
							infoHandler.rightInfo:Invalidate()
							infoHandler.UpdateStartRectPositionsInMinimap()
						end
					end
